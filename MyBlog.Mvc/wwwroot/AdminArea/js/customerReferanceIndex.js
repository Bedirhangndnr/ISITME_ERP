$(document).ready(function () {
                                ...
                                $.each(customerReferanceListDto.Data.CustomerReferances.$values,
                                    function (index, customerReferance) {
                                        const newTableRow = dataTable.row.add([
                                            ...
                                            customerReferance.Description == null ? "Açıklama Eklenmemiş" : (customerReferance.Description.length > 75 ? customerReferance.Description.substring(0, 100) : customerReferance.Description),
                                            customerReferance.Note == null ? "Not Eklenmemiş" : (customerReferance.Note.length > 75 ? customerReferance.Note.substring(0, 100) : customerReferance.Note),
    ...
    $(document).on('click',
        ...
        function (event) {
            ...
            const customerReferanceTitle = tableRow.find('td:eq(2)').text(); // table datadan 2. indexdeki değeri aldık.
            Swal.fire({
                title: tableType === 'DeletedTables' ? 'Kalıcı olarak silmek istediğinize emin misiniz?' : 'Silmek istediğinize emin misiniz?',
                text: `${customerReferanceTitle} Başlıklı CustomerReferance ${tableType === 'DeletedTables' ? 'kalıcı olarak ' : ''} Silinecektir!`,
                icon: 'warning',
            ...
            function (event) {
                ...
                $.post(actionUrl, dataToSend).done(function (data) {
                    ...
                    if (isValid) {
                        ...
                        const customerReferance = CustomerReferanceAddAjaxModel.CustomerReferanceDto.CustomerReferance;
                        const newTableRow = dataTable.row.add([
                            customerReferance.Id,
                            customerReferance.FirstName,
                            customerReferance.LastName,
    ...
    $(function () {
        ...
        placeHolderDiv.on('click',
            ...
            function (event) {
                ...
                $.post(actionUrl, dataToSend).done(function (data) {
                    const customerReferanceTypeUpdateAjaxViewModel = jQuery.parseJSON(data);
                    console.log(customerReferanceTypeUpdateAjaxViewModel);
                    const newFormBody = $('.modal-body', customerReferanceTypeUpdateAjaxViewModel.customerReferanceUpdatePartial);
                    placeHolderDiv.find('.modal-body').replaceWith(newFormBody);
                    const isValid = newFormBody.find('[name="IsValid"]').val() === 'True';
                    if (isValid) {
                        const customerReferance = customerReferanceTypeUpdateAjaxViewModel.CustomerReferanceDto.CustomerReferance
                        ...
                        toastr.success(`${customerReferanceTypeUpdateAjaxViewModel.CustomerReferanceDto.Message}`, "Başarılı İşlem!");
                    } else {
                        let summaryText = "";
                        $('#validation-summary > ul > li').each(function () {
                            let text = $(this).text();
    ...
    $(document).on('click',
        ...
        function (event) {
            ...
            Swal.fire({
                title: tableType === 'DeletedTables' ? 'Silinen CustomerReferance Geri Getirilsin Mi??' : 'Silmek istediğinize emin misiniz?',
                text: `${customerReferanceFirsName} Adlı CustomerReferance Geri Getirilecektir!`,
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
